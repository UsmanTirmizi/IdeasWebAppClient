    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://dalda-server-f80baff6d3a9.herokuapp.com/api/ideas/idea-status-count/${userid}`
        );
  ...
  const getAllUsers = async () => {
    try {
      const { data } = await axios.get(
        `https://dalda-server-f80baff6d3a9.herokuapp.com/api/auth/users-managed/${parseInt(auth.user.userid)}`
      );
      setUsers(data.users);
