...
export default function Idealstatus() {
    ...
    const getAllIdeas = async () => {
        try {
            const { data } = await axios.get(
                `https://dalda-server-f80baff6d3a9.herokuapp.com/api/ideas/idea-user/${auth.user.userid}`
            );
            setIdeas(data.ideas);
    ...
    const getAllUserIdeas = async () => {
        try {
            const { data } = await axios.get(
                `https://dalda-server-f80baff6d3a9.herokuapp.com/api/auth/users-managed/${parseInt(
                    auth.user.userid
                )}`
            );
            ...
            for (let i = 0; i < users.length; i++) {
                const { data } = await axios.get(
                    `https://dalda-server-f80baff6d3a9.herokuapp.com/api/ideas/idea-user/${users[i].userid}`
                );
    ...
    const handleApprove = (id) => {
        ...
        let config = {
            ...
            url: `https://dalda-server-f80baff6d3a9.herokuapp.com/api/ideas/update-status/${id}`,
            headers: {
